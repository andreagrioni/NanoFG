...
# except:
INPUT_BAM = 'chr14_8.candidate.fusions.reads.bam'
INPUT_VCF = '/home/grioni_andrea/lobby/playground/20200219_1355_MN17257_FAN00121_68a8fc8a_noINS.vcf'
OUTPUT_DIR = '/home/grioni_andrea/lobby/playground/candidate_fusions'

# create logging file

LOG_FILENAME = f'{Path(INPUT_BAM).name}.log'
logging.basicConfig(filename=LOG_FILENAME, filemode='w',
                    level=logging.INFO)


def logging_string(sentence, iterable):
    iterable = map(str, iterable)
    logstring = f'{sentence}' + '\t'.join(iterable)
    print(logstring)
...
def alt_convert(record):
    ...
    record.ALT = [pyvcf.model._Breakend(
        CHR2, record.INFO['END'], orientation, remoteOrientation, record.REF, True)]
    return(record)
...
def create_fasta(chr, start, end, svid, exclude, include):
    ...
    fasta = open(OUTPUT_DIR+"/"+svid+".fasta", 'a+')
...
for line in ensembl_genes.iterrows():
    index, data = line
    columns = data.tolist()
    regions.append({'id': columns[0], 'chromosome': columns[1], 'start': int(
        columns[2]), 'end': int(columns[3]), 'strand': int(columns[4]), 'biotype': columns[5]})
...
if "source" in vcf_reader.metadata:
    if vcf_reader.metadata["source"][0].lower() == "sniffles":
        vcf_type = "Sniffles"
        vcf_reader.infos['RNAMES'] = pyvcf.parser._Info(
            'RNAMES', ".", "String", "Names of reads supporting SVs (comma separated)", "Sniffles", "X")
...
for record in vcf_reader:
    ...
    logging_string('record attributes:\t', [
        record.CHROM,
        record.POS,
        strand1,
        record.ALT[0].chr,
    ...
    fusions = get_gene_overlap(record.CHROM, record.POS, strand1,
                               record.ALT[0].chr, record.ALT[0].pos, strand2, regions)
    if not fusions:
        continue
    # logging_string('record fusion is:\t', [
    #               fusions['acceptor'], fusions['donor']])
    # sys.exit()
    ...
    if 'donor' in fusions and 'acceptor' in fusions:
        ...
        if "ALT_READ_IDS" in record.INFO:
            ...
            REF_READ_IDS = record.INFO['REF_READ_IDS_1'] + \
                record.INFO['REF_READ_IDS_2']
        ...
        for donor in fusions['donor']:
            ...
            logging.info('donor gene and bp is: %s %s', donor_gene, donor_bp)
            for acceptor in fusions['acceptor']:
                acceptor_gene, acceptor_bp = acceptor.split("\t")
                logging.info('acceptor gene and bp is: %s %s',
                             acceptor_gene, acceptor_bp)
                fusion = donor_gene+"_"+acceptor_gene
                logging.info('fusion gene is: %s', fusion)

                if donor_gene != acceptor_gene:
                    logging.info('donor != acceptor, continue...')
                    ...
                    if donor_bp == '1' and acceptor_bp == '2':
                        ...
                        if donor_size > largest_donor_size:
                            ...
                            logging.info(
                                '[1,2] donor_size<=largest_donor_size, unknown outcome for fusion %s', fusion)

                        acceptor_size = abs(
                            record.ALT[0].pos-fusions['acceptor'][acceptor])
                        if acceptor_size > largest_acceptor_size:
                            ...
                            logging.info(
                                '[1,2] acceptor_size<=largest_acceptor_size, unknown outcome for fusion %s', fusion)

                    elif donor_bp == '2' and acceptor_bp == '1':
                        good_fusion = True

                        donor_size = abs(
                            record.ALT[0].pos-fusions['donor'][donor])
                        if donor_size > largest_donor_size:
                            largest_donor_size = donor_size
                            ...
                            logging.info(
                                '[2,1] donor_size<=largest_donor_size, unknown outcome for fusion %s', fusion)
                        acceptor_size = abs(
                            record.POS-fusions['acceptor'][acceptor])
                        ...
                        if acceptor_size > largest_acceptor_size:
                            ...
                            logging.info(
                                '[2,1] acceptor_size<=largest_acceptor_size, unknown outcome for fusion %s', fusion)
                    else:
                        logging.info(
                            'donor-acceptor pairs not in [1,2] or [2,1], reject fusion %s', fusion)
                else:
                    logging.info('donor == acceptor, reject fusion %s', fusion)
    logging.info('good fusion is %s', good_fusion)
    if good_fusion:
        # EXTRACT ALL THE READS THAT SUPPORT THE BREAKPOINT FOR LATER REMAPPING WITH MORE ACCURATE PARAMETERS
        create_fasta(donor_chr, donor_start, donor_end,
                     record.ID, REF_READ_IDS, SUPP_READ_IDS)
        create_fasta(acceptor_chr, acceptor_start, acceptor_end,
                     record.ID, REF_READ_IDS, SUPP_READ_IDS)
